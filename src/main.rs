...
impl VisualGrid {
    ...
    fn from(grid_cell_size: i32, grid_width: i32, grid_height: i32) -> Self {
        Self {
            grid_cell_size,
            grid_width,
            grid_height,
...
impl From<&VisualGrid> for Rect {
    fn from(vg: &VisualGrid) -> Self {
        let rec = Rect::new(
            ((vg.grid_width - 1) / 2 * vg.grid_cell_size)
                .try_into()
                .unwrap(),
            ((vg.grid_height - 1) / 2 * vg.grid_cell_size)
                .try_into()
                .unwrap(),
...
impl GolGrid {
    ...
    fn complement_val(&mut self, coord: &Coord) {
        ...
        if self.grid[self.curr_grid][index] {
            self.alive_cells.remove(
                self.alive_cells
                    .iter()
                    .position(|x| *x == *coord)
                    .expect("Coord not found!"),
            );
        } else {
    ...
    fn get_index(coord: &Coord) -> usize {
        let index: usize = (coord.x + coord.y * 10_000)
            .try_into()
            .expect("Failed to convert coord into index!");
    ...
    fn simulate(&mut self) {
        ...
        for (_, coord) in cells_to_check.iter().enumerate() {
            ...
            for neighbour in sanitize(curr_coor.get_neighbours()) {
                if self.get_val(&neighbour) {
                    //there is a alive neighbour
                    num_neigh += 1;
                }
            }
...
pub fn main() {
    ...
    let window = video_subsystem
        .window(
            "etude-gol",
            vg.window_width.try_into().unwrap(),
            vg.window_height.try_into().unwrap(),
        )
        .position_centered()
        .build()
        .unwrap();
    ...
    'running: loop {
        ...
        for x in (0..1 + vg.grid_width * vg.grid_cell_size)
            .step_by(vg.grid_cell_size.try_into().unwrap())
        {
            let _ = canvas.draw_line(Point::new(x, 0), Point::new(x, vg.window_height));
        }
        //horizontal lines
        for y in (0..1 + vg.grid_height * vg.grid_cell_size)
            .step_by(vg.grid_cell_size.try_into().unwrap())
        {
        ...
        for x in
            (0..vg.grid_width * vg.grid_cell_size).step_by(vg.grid_cell_size.try_into().unwrap())
        {
            for y in (0..1 + vg.grid_height * vg.grid_cell_size)
                .step_by(vg.grid_cell_size.try_into().unwrap())
            {
